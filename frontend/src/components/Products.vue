...
//const totalProdutos = () => produtos.value.length;
...
<template>
  <section class="max-w-[1240px] m-[0_auto] p-[20px] mt-[20px]">
    ...
    <div class="flex flex-wrap justify-center mt-[40px] gap-[10px]">
      <RouterLink v-for="produto in paginatedProdutos" :key="produto.id" :to="`/produto/${produto.slug}`" class="relative bg-[white] w-[265px] rounded-[4px] hover:cursor-pointer border-[2px] border-[transparent] hover:border-[2px] hover:border-primary-color hover:z-[80] group">
        <div>
          <div class="w-full h-[190px] relative flex items-center justify-center mb-[10px] bg-[#F7F8F8] rounded-[3px]">
          ...
          <div class="flex flex-col gap-[8px] p-[16px] pt-0">
            <h2 class="text-[#0f1111] text-[18px] text-base line-clamp-2 group-hover:line-clamp-2">
              {{ produto.produto }}
            </h2>
            <div>
